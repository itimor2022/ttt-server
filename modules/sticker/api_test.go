...
func TestAddUserCustomSticker(t *testing.T) {
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/sticker/user", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"width":  230,
		"height": 300,
		"path":   "xx111",
	}))))
...
func TestGetUserCustomSticker(t *testing.T) {
	...
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	err = f.db.insertUserSticker(&customModel{
		Width:   200,
		Height:  300,
		Path:    "sdd",
		UID:     testutil.UID,
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/sticker/user", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
...
func TestDeleteCustomSticker(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("DELETE", "/v1/sticker/user", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"paths": list,
	}))))
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
...
func TestUserDeleteByCategory(t *testing.T) {
	...
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	err = f.db.insertUserCategory(&categoryModel{
		UID:      testutil.UID,
		SortNum:  1,
		Category: "111",
	})
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("DELETE", "/v1/sticker/remove", bytes.NewReader([]byte(util.ToJson(map[string]interface{}{
		"category": "111",
	}))))
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
...
func TestUserAddByCategory(t *testing.T) {
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/v1/sticker/user/111", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
}
...
func TestGetUserCategory(t *testing.T) {
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/sticker/user/category", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
...
func TestGetStickerWithCategory(t *testing.T) {
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/sticker/user/sticker?category=111", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"category":"2232"`))
...
func TestStoreList(t *testing.T) {
	...
	err := testutil.CleanAllTables(ctx)
	assert.NoError(t, err)
	err = f.db.insertStore(&storeModel{
		Title:    "tss",
		Category: "css",
		Cover:    "coverss",
		Desc:     "dss",
	...
	assert.NoError(t, err)
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/sticker/store?page_index=2&page_size=10", nil)
	req.Header.Set("token", testutil.Token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
