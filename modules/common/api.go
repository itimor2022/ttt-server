...
func (cn *Common) Route(r *wkhttp.WKHttp) {
	...
	commonNoAuth := r.Group("/v1/common")
	{
		...
		commonNoAuth.GET("/info", cn.getInfo)                  // 获取信息
	}
...
func (cn *Common) appConfig(c *wkhttp.Context) {
	...
	if versionI64 != 0 && int(versionI64) >= appConfigM.Version {
		...
		c.JSON(http.StatusOK, map[string]interface{}{
			...
			"ai_invest_url":         appConfigM.AIInvestUrl,
			"insight_url":           appConfigM.InsightUrl,
			"ai_invest_on":          appConfigM.AIInvestOn,
			"insight_connection_on": appConfigM.InsightConnectionOn,
		})
		return
	}
	...
	c.JSON(http.StatusOK, map[string]interface{}{
		...
		"web_url":                             cn.ctx.GetConfig().External.WebLoginURL,
		"revoke_second":                       revokeSecond,
		"register_invite_on":                  appConfigM.RegisterInviteOn,
		"send_welcome_message_on":             appConfigM.SendWelcomeMessageOn,
		"invite_system_account_join_group_on": appConfigM.InviteSystemAccountJoinGroupOn,
		"register_user_must_complete_info_on": appConfigM.RegisterUserMustCompleteInfoOn,
		"can_modify_api_url":                  appConfigM.CanModifyApiUrl,
		"moments_off":                         appConfigM.MomentsOff,
		"ai_invest_url":                       appConfigM.AIInvestUrl,
		"insight_url":                         appConfigM.InsightUrl,
		"ai_invest_on":                        appConfigM.AIInvestOn,
		"insight_connection_on":               appConfigM.InsightConnectionOn,
...
// 获取信息
func (cn *Common) getInfo(c *wkhttp.Context) {
	c.String(http.StatusOK, "大雕NB TG：@laowang79888")
}
