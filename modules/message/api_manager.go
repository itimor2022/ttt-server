...
func (m *Manager) Route(r *wkhttp.WKHttp) {
	auth := r.Group("/v1/manager", m.ctx.AuthMiddleware(r))
	{
		...
		auth.GET("/message/channels", m.channelList)                  // 有消息记录的频道列表
	}
}
...
// 有消息记录的频道列表
func (m *Manager) channelList(c *wkhttp.Context) {
	err := c.CheckLoginRole()
	if err != nil {
		c.ResponseError(err)
		return
	...
	pageIndex, pageSize := c.GetPage()
	channelType := c.Query("channel_type") // 1: 单聊, 2: 群聊, 空: 全部
	keyword := c.Query("keyword")          // 搜索关键字

	list, err := m.managerDB.queryChannelList(uint64(pageIndex), uint64(pageSize), channelType, keyword)
	if err != nil {
		m.Error("查询频道列表错误", zap.Error(err))
		c.ResponseError(errors.New("查询频道列表错误"))
		return
	}
	count, err := m.managerDB.queryChannelCount(channelType, keyword)
	if err != nil {
		m.Error("查询频道总数错误", zap.Error(err))
		c.ResponseError(errors.New("查询频道总数错误"))
		return
	}

	// 获取频道名称和头像信息
	result := make([]*channelListVO, 0)
	for _, item := range list {
		...
		if item.ChannelType == 1 {
			...
			// 群聊 - 获取群名称
			group, err := m.groupService.GetGroupWithGroupNo(item.ChannelID)
			if err == nil && group != nil {
				vo.ChannelName = group.Name
			} else {
				vo.ChannelName = item.ChannelID
			}
		}
...
type channelListVO struct {
	...
	ChannelType  uint8  `json:"channel_type"`  // 频道类型 1:单聊 2:群聊
