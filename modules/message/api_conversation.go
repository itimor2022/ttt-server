...
func (co *Conversation) syncUserConversation(c *wkhttp.Context) {
	...
	// 查询全局置顶设置
	type topUserConfig struct {
		UID         string
		IsTop       int
		TopPriority int
	}
	topUserConfigs := make(map[string]*topUserConfig)
	rows, err := co.ctx.DB().Query("SELECT uid, is_top, top_priority FROM user_top_setting WHERE is_top=?", 1)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var uid string
			var isTop, topPriority int
			if err := rows.Scan(&uid, &isTop, &topPriority); err == nil {
				topUserConfigs[uid] = &topUserConfig{
					UID:         uid,
					IsTop:       isTop,
	...
	// ---------- 为置顶用户添加会话条目（即使没有聊天记录） ----------
	// 获取所有置顶用户
	topUserUIDs := make([]string, 0)
	for uid := range topUserConfigs {
		topUserUIDs = append(topUserUIDs, uid)
	}

	// 检查置顶用户是否已经在会话列表中
	existingChannelIDs := make(map[string]bool)
	for _, conversation := range conversations {
		existingChannelIDs[conversation.ChannelID] = true
	}

	// 为不在会话列表中的置顶用户创建会话条目
	if len(topUserUIDs) > 0 {
		// 查询置顶用户的详细信息
		topUsers, err := co.userService.GetUserDetails(topUserUIDs, loginUID)
		if err == nil && len(topUsers) > 0 {
			for _, topUser := range topUsers {
				// 检查是否已经在会话列表中
				if !existingChannelIDs[topUser.UID] {
					// 创建新的会话条目
					newConversation := &config.SyncUserConversationResp{
						ChannelID:    topUser.UID,
	...
	if len(conversations) > 0 {
		for _, conversation := range conversations {
			...
			if len(syncUserConversationResp.Recents) > 0 {
				...
				// 对于置顶用户，即使没有聊天记录也要显示
				if conversation.ChannelType == common.ChannelTypePerson.Uint8() {
					if _, ok := topUserConfigs[conversation.ChannelID]; ok {
						syncUserConversationResps = append(syncUserConversationResps, syncUserConversationResp)
					}
				}
			}
	...
	// 对会话列表进行排序：官方账号 > 置顶用户 > 普通用户（按时间戳）
	sort.Slice(syncUserConversationResps, func(i, j int) bool {
		// 检查是否为官方账号
		iIsOfficial := false
		if syncUserConversationResps[i].ChannelType == common.ChannelTypePerson.Uint8() {
			userDetail := userMap[syncUserConversationResps[i].ChannelID]
			if userDetail != nil && userDetail.Category == "official" {
				iIsOfficial = true
			}
		}

		jIsOfficial := false
		if syncUserConversationResps[j].ChannelType == common.ChannelTypePerson.Uint8() {
			userDetail := userMap[syncUserConversationResps[j].ChannelID]
			if userDetail != nil && userDetail.Category == "official" {
				jIsOfficial = true
			}
		}
		...
		iPriority := 0
		if syncUserConversationResps[i].ChannelType == common.ChannelTypePerson.Uint8() {
			if config, ok := topUserConfigs[syncUserConversationResps[i].ChannelID]; ok {
				iIsTop = true
				iPriority = config.TopPriority
			}
		}
		...
		jPriority := 0
		if syncUserConversationResps[j].ChannelType == common.ChannelTypePerson.Uint8() {
			if config, ok := topUserConfigs[syncUserConversationResps[j].ChannelID]; ok {
				jIsTop = true
				jPriority = config.TopPriority
			}
		}
		...
		// 官方账号之间按时间戳排序
		if iIsOfficial && jIsOfficial {
			return syncUserConversationResps[i].Timestamp > syncUserConversationResps[j].Timestamp
		}
