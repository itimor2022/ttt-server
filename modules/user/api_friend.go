...
func NewFriend(ctx *config.Context) *Friend {
	...
	f.ctx.AddEventListener(event.EventUserRegister, f.handleUserRegisterWithTopAccounts)
	return f
}
...
func (f *Friend) delete(c *wkhttp.Context) {
	...
	var isTop int
	var err error
	_, err = f.ctx.DB().Select("is_top").From("user_top_setting").Where("uid = ?", uid).Load(&isTop)
	if err == nil && isTop == 1 {
		c.ResponseError(errors.New("官方账号不能被删除"))
		return
	}
...
func (f *Friend) friendApply(c *wkhttp.Context) {
	...
	// 检查用户是否有添加好友的权限
	if loginUserInfo.Category == "normal" {
		// 查询应用配置，检查普通用户是否可以添加好友
		var appConfigModel struct {
			NormalUserCanAddFriend int
		}
	...
	// 查询双方的好友关系
	friendModels, err := f.db.queryTwoWithUID(fromUID, req.ToUID)
	if err != nil {
	...
	var fromFriend *FriendModel // fromUID -> toUID
	var toFriend *FriendModel   // toUID -> fromUID
	for _, fm := range friendModels {
		if fm.UID == fromUID && fm.ToUID == req.ToUID {
			fromFriend = fm
		} else if fm.UID == req.ToUID && fm.ToUID == fromUID {
			toFriend = fm
		}
	}

	// 检查是否双方都是好友
	isFriendLoginUser := fromFriend != nil && fromFriend.IsDeleted == 0
	isFriendToUser := toFriend != nil && toFriend.IsDeleted == 0
	...
	// 处理单向好友恢复的情况：如果对方认为是好友（toFriend.IsDeleted=0），而申请者已删除（fromFriend.IsDeleted=1）
	// 则直接恢复好友关系，不需要对方再次确认
	if toFriend != nil && toFriend.IsDeleted == 0 && fromFriend != nil && fromFriend.IsDeleted == 1 {
		f.Info("检测到单向好友恢复情况，直接恢复好友关系",
			zap.String("from_uid", fromUID), zap.String("to_uid", req.ToUID))

		// 直接恢复好友关系
		version := f.ctx.GenSeq(common.FriendSeqKey)
		err = f.db.updateRelationship2Tx(fromUID, req.ToUID, 0, 0, version, nil)
		if err != nil {
			f.Error("恢复好友关系失败", zap.Error(err))
			c.ResponseError(errors.New("恢复好友关系失败"))
			return
		}
		// 同时重置对方的单向标记
		err = f.db.updateAlone(req.ToUID, fromUID, 0)
		if err != nil {
			f.Error("更新单向好友标记失败", zap.Error(err))
			c.ResponseError(errors.New("更新单向好友标记失败"))
			return
		}
		...
		if err != nil {
			f.Warn("发送好友恢复通知失败", zap.Error(err))
		}
...
func (f *Friend) friendSure(c *wkhttp.Context) {
	...
	// 查询双方的好友记录
	friendModels, err := f.db.queryTwoWithUID(loginUID, applyUID)
	if err != nil {
		f.Error("查询好友记录失败！", zap.Error(err), zap.String("uid", loginUID), zap.String("toUid", applyUID))
		c.ResponseError(errors.New("查询好友记录失败！"))
		return
	}

	// 分析双方的好友记录
	var applyFriendModel *FriendModel // loginUID -> applyUID (接受者->申请者)
	var loginFriendModel *FriendModel // applyUID -> loginUID (申请者->接受者)
	for _, fm := range friendModels {
		if fm.UID == loginUID && fm.ToUID == applyUID {
			applyFriendModel = fm
		} else if fm.UID == applyUID && fm.ToUID == loginUID {
			loginFriendModel = fm
		}
	}
	...
	// 处理 loginUID -> applyUID 的好友记录（接受者->申请者）
	if applyFriendModel == nil {
		...
		f.Info("恢复好友关系", zap.String("uid", loginUID), zap.String("to_uid", applyUID),
			zap.Int("old_is_deleted", applyFriendModel.IsDeleted), zap.Int("old_is_alone", applyFriendModel.IsAlone))
	}

	// 处理 applyUID -> loginUID 的好友记录（申请者->接受者）
	if loginFriendModel == nil {
		...
		// 无论当前状态如何，都重置为正常好友关系（解决删除后重新添加is_alone=1的问题）
		...
		f.Info("恢复好友关系", zap.String("uid", applyUID), zap.String("to_uid", loginUID),
			zap.Int("old_is_deleted", loginFriendModel.IsDeleted), zap.Int("old_is_alone", loginFriendModel.IsAlone))
	}
	// 发布好友确认事件
	eventID, err := f.ctx.EventBegin(&wkevent.Data{
...
func (f *Friend) friendSearch(c *wkhttp.Context) {
	...
	// 检查用户是否有添加好友的权限
	loginUserInfo, err := f.userDB.QueryByUID(uid)
	if err != nil {
		f.Error("查询用户信息错误", zap.Error(err))
		c.ResponseError(errors.New("查询用户信息错误"))
		return
	...
	if loginUserInfo == nil || loginUserInfo.IsDestroy == 1 || loginUserInfo.Status != 1 {
		f.Error("登录用户不存在！", zap.String("uid", uid))
		c.ResponseError(errors.New("登录用户不存在！"))
		return
	}
	if loginUserInfo.Category == "normal" {
		...
		// 如果普通用户不允许添加好友，则返回网络连接失败
		if appConfigModel.NormalUserCanAddFriend != 1 {
			f.Error("普通用户没有添加好友的权限", zap.String("uid", uid))
