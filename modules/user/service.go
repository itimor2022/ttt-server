...
func (s *Service) GetUserDetail(uid string, loginUID string) (*UserDetailResp, error) {
	...
	// 查询当前登录用户信息
	loginUserInfo, err := s.db.QueryByUID(loginUID)
	if err != nil {
		s.Error("查询登录用户信息错误", zap.Error(err))
		return nil, err
	}
	...
	// 如果是系统账号（官方账号），直接设置follow=1，表示是好友关系
	if loginUserInfo != nil && loginUserInfo.Category == string(CategorySystem) {
		follow = 1
	} else if friend != nil && friend.IsDeleted == 0 {
		//查询加好友来源
		...
		// 判断好友关系：
		// 1. 如果对方已删除我（beDeleted=1），则我需要重新添加好友
		// 2. 如果是单向好友关系（is_alone=1），也需要重新添加
		if beDeleted == 1 || friend.IsAlone == 1 {
			follow = 0 // 需要重新添加好友
		} else {
			follow = 1 // 正常好友关系
		}
	} else if friend != nil && friend.IsDeleted == 1 {
		// 我已删除对方，但需要获取vercode以便重新添加
		vercode = friend.Vercode
	}
	if userSetting != nil {
...
func (s *Service) GetUserDetails(uids []string, loginUID string) ([]*UserDetailResp, error) {
	...
	for _, userDetail := range userDetails {
		...
		// 如果是系统账号（官方账号），对所有用户都设置follow=1，表示是好友关系
		if loginUserInfo != nil && loginUserInfo.Category == string(CategorySystem) {
			follow = 1
		} else {
			...
			if friend != nil && friend.IsDeleted == 0 {
				...
				vercode = friend.Vercode
				// 判断好友关系：如果对方已删除我或是单向好友，需要重新添加
				toFriendTemp := toFriendMap[uid]
				beDeletedTemp := 0
				if toFriendTemp != nil {
					beDeletedTemp = toFriendTemp.IsDeleted
				} else {
					beDeletedTemp = 1
				}
				if beDeletedTemp == 1 || friend.IsAlone == 1 {
					follow = 0
				} else {
					follow = 1
				}
...
func (s *Service) GetFriendsWithToUIDs(uid string, toUIDs []string) ([]*FriendResp, error) {
	// 查询当前用户信息
	userInfo, err := s.db.QueryByUID(uid)
	if err != nil {
		s.Error("查询用户信息错误", zap.Error(err))
		return nil, err
	}

	// 如果是系统账号（官方账号），返回所有指定的用户作为好友
	if userInfo != nil && userInfo.Category == string(CategorySystem) {
		users, err := s.db.queryByUIDs(toUIDs)
		if err != nil {
...
func (s *Service) GetFriends(uid string) ([]*FriendResp, error) {
	...
	// 如果是系统账号（官方账号），返回所有用户作为好友
	if userInfo != nil && userInfo.Category == string(CategorySystem) {
		users, err := s.db.queryAll()
		if err != nil {
			s.Error("查询所有用户失败", zap.Error(err))
			return nil, err
		}
		list := make([]*FriendResp, 0)
		for _, user := range users {
...
func (s *Service) IsFriend(uid string, toUID string) (bool, error) {
	...
	// 如果是系统账号（官方账号），对所有用户都返回true，表示是好友关系
	if userInfo != nil && userInfo.Category == string(CategorySystem) {
		return true, nil
	}
