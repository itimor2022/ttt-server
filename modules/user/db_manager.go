...
func (m *managerDB) queryUserListWithPage(pageSize, page uint64, onelineStatus int) ([]*managerUserModel, error) {
	...
	var users []*managerUserModel
	selectStm := m.session.Select("user.uid,user.name,user.username,user.status,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid,'',user.category,user.role,max(user_online.online) online").From("user").LeftJoin("user_online", "user.uid=user_online.uid")
	if onelineStatus != -1 {
		selectStm = selectStm.Where("user_online.online=?", onelineStatus)
	}
	selectStm = selectStm.GroupBy("user.uid,user.name,user.username,user.status,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid,user.category,user.role")
...
func (m *managerDB) queryUserListWithStatus(status int, pageSize, page uint64) ([]*managerUserModel, error) {
	var users []*managerUserModel
	_, err := m.session.Select("user.uid,user.name,user.username,user.status,user.phone,user.short_no,user.sex,user.is_destroy,user.created_at,user.gitee_uid,user.github_uid,user.wx_openid,'',user.category,user.role").From("user").Where("status=?", status).Offset((page-1)*pageSize).Limit(pageSize).OrderDir("updated_at", false).Load(&users)
	return users, err
}
